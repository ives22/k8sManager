<template>
    <div class="statefulset">
                ...
                <div>
                    ...
                    <el-card class="statefulset-head-card" shadow="never" :body-style="{ padding: '10px' }">
                        <el-row>
                            ...
                            <el-col :span="2">
                                <div >
                                    <!-- 点击后打开抽屉，填入创建statefulset需要的数据 -->
                                    <el-button disabled style="border-radius:2px;" 
                                    icon="Edit" 
                                    type="primary"
                                    v-loading.fullscreen.lock="fullscreenLoading"
                                    @click="createStatefulsetDrawer = true">创建</el-button>
                                </div>
                            </el-col>
                            <!-- 搜索输入框和搜索按钮 -->
                            <el-col :span="6">
                                <div>
                                    <!-- 搜索输入框 -->
                                    <el-input class="statefulset-head-search" 
            ...
            <el-col :span="24">
                <div>
                    <el-card class="statefulset-body-card" shadow="never" :body-style="{padding:'5px'}">
                        ...
                        <el-table style="width: 100%; font-size:12px; margin-bottom:10px" :data="statefulsetList" v-loading="appLoading">
                            <!-- 最左侧留出20px的宽度，更加美观 -->
                            ...
                            <el-table-column align=left label="Statefulset名">
                                ...
                                <template v-slot="scope">
                                    <!-- <a href="" class="statefulset-body-statefulsetname">{{scope.row.metadata.name}}</a> -->
                            ...
                            <el-table-column align=center label="容器组">
                                ...
                                <template v-slot="scope">
                                    <span>{{ scope.row.status.readyReplicas>0?scope.row.status.readyReplicas:0  }} / {{ scope.row.spec.replicas>0?scope.row.spec.replicas:0 }} </span>
                                </template>
                            </el-table-column>
                            <!-- 创建时间 -->
        ...
        <el-Drawer v-model="createStatefulsetDrawer"
        :direction="direction"
        :before-close="handleClose">
            ...
            <template #default>
                ...
                <el-row type="flex" justify="center">
                    <el-col :span="20">
                        ...
                        <!-- rules 定义form表单校验规则 -->
                        <el-form ref="createStatefulset" :rules="createStatefulsetRules" :model="createStatefulset" label-width="80px">
                            <!-- prop用于rules中的校验规则的key -->
                            <el-form-item class="statefulset-create-form" label="名称" prop="name">
                                <el-input v-model="createStatefulset.name"></el-input>
                            </el-form-item>
                            <el-form-item class="statefulset-create-form" label="命名空间" prop="namespace">
                                <el-select v-model="createStatefulset.namespace" filterable placeholder="请选择">
                                    <el-option v-for="(item, index) in namespaceList" :key="index"
                                        :label="item.metadata.name" :value="item.metadata.name">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item class="statefulset-create-form" label="副本数" prop="replicas">
                                <el-input-number v-model="createStatefulset.replicas" :min="1" :max="10"></el-input-number>
                            ...
                            <el-form-item class="statefulset-create-form" label="镜像" prop="image">
                                <el-input v-model="createStatefulset.image"></el-input>
                            </el-form-item>
                            <el-form-item class="statefulset-create-form" label="标签" prop="label_str">
                                <el-input v-model="createStatefulset.label_str" placeholder="示例: project=ms,app=gateway"></el-input>
                            </el-form-item>
                            <!-- 下拉框，用于规格的选择，之后用/分割，得到cpu和内存 -->
                            <el-form-item class="statefulset-create-form" label="资源配额" prop="resource">
                                <el-select v-model="createStatefulset.resource" placeholder="请选择">
                            ...
                            <el-form-item class="statefulset-create-form" label="容器端口" prop="container_port">
                                <el-input v-model="createStatefulset.container_port" placeholder="示例: 80"></el-input>
                            </el-form-item>
                            <el-form-item class="statefulset-create-form" label="健康检查" prop="health">
                                <el-switch v-model="createStatefulset.health_check" />
                            </el-form-item>
                            <el-form-item class="statefulset-create-form" label="检查路径" prop="healthPath">
                                <el-input v-model="createStatefulset.health_path" placeholder="示例: /health"></el-input>
                            </el-form-item>
                        </el-form>
                    </el-col>
                </el-row>
            ...
            <!-- footer，处理提交和取消 -->
            <template #footer>
                <el-button @click="createStatefulsetDrawer = false">取消</el-button>
                <el-button type="primary" @click="submitForm('createStatefulset')">立即创建</el-button>
            </template>
        </el-Drawer>
...
export default {
    ...
    methods: {
        ...
        // 创建Statefulset，提交表单，校验参数合法性
        submitForm(formName){
            this.$refs[formName].validate((valid)=>{
                if (valid) {
                    this.createStatefulsetFunc()
                } else {
                    false
                }
            })
        },
        // 创建statefulset，加Func的原因是因为createStatefulset用于属性了
        createStatefulsetFunc(){
            ...
            //将statefulset的规格转成cpu和memory
            let resourceList = this.createStatefulset.resource.split("/")
            cpu = resourceList[0]
            memory = resourceList[1] + "Gi"

            // 赋值
            this.createStatefulsetData.params = this.createStatefulset
            this.createStatefulsetData.params.container_port = parseInt(this.createStatefulset.container_port)
            this.createStatefulsetData.params.label = label
            this.createStatefulsetData.params.cpu = cpu
            this.createStatefulsetData.params.memory = memory
            this.createStatefulsetData.params.cluster = this.selectClusterValue

            // 发起请求
            httpClient.post(this.createStatefulsetData.url, this.createStatefulsetData.params).then(res=>{
            ...
            // 关闭抽屉
            this.createStatefulsetDrawer = false
        },
        // 获取Statefulset列表
        getStatefulsets() {
            // 表格加载动画开启
            this.appLoading = true
            //getStatefulsetsData是用于发起statefulset列表请求的专用的对象，里面有url和params参数,以下是赋值
            ...
            this.getStatefulsetsData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getStatefulsetsData.url, {params: this.getStatefulsetsData.params}).then(res => {
        ...
        // 获取Statefulset详情，e参数标识传入的scope插槽，.row是该行的数据
        getStatefulsetDetail(e){
            ...
            this.getStatefulsetDetailData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getStatefulsetDetailData.url, {params: this.getStatefulsetDetailData.params}).then(res => {
                //响应成功，获得statefulset详情
        ...
        updateStatefulset(){
            ...
            this.updateStatefulsetData.params.cluster = this.selectClusterValue
            httpClient.put(this.updateStatefulsetData.url, this.updateStatefulsetData.params).then(res => {
                this.$message.success({
                    message: res.msg
                })
                //更新后重新获取列表
        ...
        delStatefulset(e){
            ...
            this.deleteStatefulsetData.params.statefulset_name = e.row.metadata.name 
            this.deleteStatefulsetData.params.cluster = this.selectClusterValue
            httpClient.delete(this.deleteStatefulsetData.url, {data: this.deleteStatefulsetData.params}).then(res => {
                this.$message.success({
                    message: res.msg
                })
                // 删除后重新获取列表
        ...
        handleConfirm(obj, operateName, fn){
            ...
            this.$confirm(this.confirmContent, '提示', {
                ...
                //restartStatefulset(e)或者delStatefulset(e)
                fn(obj)
            }).catch(()=>{
                this.$message.info({
                    message: '已取消操作'
...
<style scoped>
/* 卡片属性 */
.statefulset-head-card,
.statefulset-body-card {
    border-radius: 1px;
    margin-bottom: 5px;
}
/* 搜索框 */
