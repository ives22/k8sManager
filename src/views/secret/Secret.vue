<template>
    <div class="secret">
                ...
                <div>
                    ...
                    <el-card class="secret-head-card" shadow="never" :body-style="{ padding: '10px' }">
                        <el-row>
                            ...
                            <el-col :span="2">
                                <div>
                                    <!-- 点击后打开抽屉，填入创建secret需要的数据 -->
                                    <el-button style="border-radius:2px;" 
                                    icon="Edit" 
                                    type="primary"
                                    disabled
                                    v-loading.fullscreen.lock="fullscreenLoading"
                                    @click="createSecretDrawer = true">创建</el-button>
                                </div>
                            </el-col>
                            <!-- 搜索输入框和搜索按钮 -->
                            <el-col :span="6">
                                <div>
                                    <!-- 搜索输入框 -->
                                    <el-input class="secret-head-search" 
            ...
            <el-col :span="24">
                <div>
                    <el-card class="secret-body-card" shadow="never" :body-style="{padding:'5px'}">
                        ...
                        <el-table style="width: 100%; font-size:12px; margin-bottom:10px" :data="secretList" v-loading="appLoading">
                            <!-- 最左侧留出20px的宽度，更加美观 -->
        ...
        <el-Drawer v-model="createSecretDrawer"
        :direction="direction"
        :before-close="handleClose">
            ...
            <template #default>
                ...
                <el-row type="flex" justify="center">
                    <el-col :span="20">
                        ...
                        <!-- rules 定义form表单校验规则 -->
                        <el-form ref="createSecret" :rules="createSecretRules" :model="createSecret" label-width="80px">
                            <!-- prop用于rules中的校验规则的key -->
                            <el-form-item class="secret-create-form" label="名称" prop="name">
                                <el-input v-model="createSecret.name"></el-input>
                            </el-form-item>
                            <el-form-item class="secret-create-form" label="命名空间" prop="namespace">
                                <el-select v-model="createSecret.namespace" filterable placeholder="请选择">
                                    <el-option v-for="(item, index) in namespaceList" :key="index"
                                        :label="item.metadata.name" :value="item.metadata.name">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item class="secret-create-form" label="副本数" prop="replicas">
                                <el-input-number v-model="createSecret.replicas" :min="1" :max="10"></el-input-number>
                            ...
                            <el-form-item class="secret-create-form" label="镜像" prop="image">
                                <el-input v-model="createSecret.image"></el-input>
                            </el-form-item>
                            <el-form-item class="secret-create-form" label="标签" prop="label_str">
                                <el-input v-model="createSecret.label_str" placeholder="示例: project=ms,app=gateway"></el-input>
                            </el-form-item>
                            <!-- 下拉框，用于规格的选择，之后用/分割，得到cpu和内存 -->
                            <el-form-item class="secret-create-form" label="资源配额" prop="resource">
                                <el-select v-model="createSecret.resource" placeholder="请选择">
                            ...
                            <el-form-item class="secret-create-form" label="容器端口" prop="container_port">
                                <el-input v-model="createSecret.container_port" placeholder="示例: 80"></el-input>
                            </el-form-item>
                            <el-form-item class="secret-create-form" label="健康检查" prop="health">
                                <el-switch v-model="createSecret.health_check" />
                            </el-form-item>
                            <el-form-item class="secret-create-form" label="检查路径" prop="healthPath">
                                <el-input v-model="createSecret.health_path" placeholder="示例: /health"></el-input>
                            </el-form-item>
                        </el-form>
                    </el-col>
                </el-row>
            ...
            <!-- footer，处理提交和取消 -->
            <template #footer>
                <el-button @click="createSecretDrawer = false">取消</el-button>
                <el-button type="primary" @click="submitForm('createSecret')">立即创建</el-button>
            </template>
        </el-Drawer>
...
export default {
    ...
    methods: {
        ...
        // 创建secret，加Func的原因是因为createSecret用于属性了
        createSecretFunc(){
            // 正则匹配，验证label的合法性
            ...
            //将secret的规格转成cpu和memory
            let resourceList = this.createSecret.resource.split("/")
            cpu = resourceList[0]
            memory = resourceList[1] + "Gi"

            // 赋值
            this.createSecretData.params = this.createSecret
            this.createSecretData.params.container_port = parseInt(this.createSecret.container_port)
            this.createSecretData.params.label = label
            this.createSecretData.params.cpu = cpu
            this.createSecretData.params.memory = memory
            this.createSecretData.params.cluster = this.selectClusterValue


            // 发起请求
            httpClient.post(this.createSecretData.url, this.createSecretData.params).then(res=>{
            ...
            // 关闭抽屉
            this.createSecretDrawer = false
        },
        // 获取Secret列表
        getSecrets() {
            // 表格加载动画开启
            this.appLoading = true
            //getSecretsData是用于发起secret列表请求的专用的对象，里面有url和params参数,以下是赋值
            ...
            this.getSecretsData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getSecretsData.url, {params: this.getSecretsData.params}).then(res => {
        ...
        // 获取Secret详情，e参数标识传入的scope插槽，.row是该行的数据
        getSecretDetail(e){
            //getSecretsData是用于发起secret列表请求的专用的对象，里面有url和params参数,以下是赋值
            ...
            this.getSecretDetailData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getSecretDetailData.url, {params: this.getSecretDetailData.params}).then(res => {
                //响应成功，获得secret详情
        ...
        updateSecret(){
            ...
            this.updateSecretData.params.cluster = this.selectClusterValue
            httpClient.put(this.updateSecretData.url, this.updateSecretData.params).then(res => {
                this.$message.success({
                    message: res.msg
                })
                //更新后重新获取列表
        ...
        delSecret(e){
            ...
            this.deleteSecretData.params.secret_name = e.row.metadata.name 
            this.deleteSecretData.params.cluster = this.selectClusterValue
            httpClient.delete(this.deleteSecretData.url, {data: this.deleteSecretData.params}).then(res => {
                this.$message.success({
                    message: res.msg
                })
                // 删除后重新获取列表
