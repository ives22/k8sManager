<template>
    <div class="service">
        <el-row>
            <!-- 1 头部1 -->
            <el-col :span="24">
                <div>
                    <el-card class="service-head-card" shadow="never" :body-style="{ padding: '10px' }">
                        <el-row>
                            <!-- 1.1 命名空间 -->
                            <el-col :span="6">
                                <div>
                                    <span>命名空间: </span>
                                    <!-- 下拉框 -->
                                    <!-- filterable：带搜索功能 -->
                                    <!-- placeholder 默认提示 -->
                                    <!-- label 显示内容 -->
                                    <!-- value 绑定到v-model的值中 -->
                                    <el-select v-model="namespaceValue" filterable placeholder="请选择">
                                        <el-option v-for="(item, index) in namespaceList" :key="index"
                                            :label="item.metadata.name" :value="item.metadata.name">
                                        </el-option>
                                    </el-select>
                                </div>
                            </el-col>
                            <!-- 1.2 刷新按钮 -->
                            <el-col :span="2" :offset="16">
                                <div>
                                    <el-button style="border-radius: 2px;" icon="Refresh" plain @click="getServices()">刷新</el-button>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
            </el-col>


            <!-- 头部2 -->
            <el-col :span="24">
                <div>
                    <!-- 包一层卡片 -->
                    <el-card class="service-head-card" shadow="never" :body-style="{ padding: '10px' }">
                        <el-row>
                            <!-- 创建按钮 -->
                            <el-col :span="2">
                                <div>
                                    <!-- 点击后打开抽屉，填入创建service需要的数据 -->
                                    <el-button style="border-radius:2px;" 
                                    icon="Edit" 
                                    type="primary"
                                    v-loading.fullscreen.lock="fullscreenLoading"
                                    @click="createServiceDrawer = true">创建</el-button>
                                </div>
                            </el-col>
                            <!-- 搜索输入框和搜索按钮 -->
                            <el-col :span="6">
                                <div>
                                    <!-- 搜索输入框 -->
                                    <el-input class="service-head-search" 
                                    clearable placeholder="请输入"
                                    v-model="searchInput"></el-input>
                                    <!-- 搜索按钮 -->
                                    <el-button style="border-radius:2px;" icon="Search" type="primary" plain @click="getServices()">搜索</el-button>
                                </div>
                            </el-col>
                        </el-row>
                    </el-card>
                </div>
            </el-col>


            <!-- 数据表格 -->
            <el-col :span="24">
                <div>
                    <el-card class="service-body-card" shadow="never" :body-style="{padding:'5px'}">
                        <!-- 数据表格 -->
                        <!-- v-loading用于加载时的loading动画 -->
                        <el-table style="width: 100%; font-size:12px; margin-bottom:10px" :data="serviceList" v-loading="appLoading">
                            <!-- 最左侧留出20px的宽度，更加美观 -->
                            <el-table-column width="20"></el-table-column>
                            <!-- service名字 -->
                            <el-table-column align=left label="Service名">
                                <!-- 插槽，scope.row获取当前行的数据 -->
                                <template v-slot="scope">
                                    <!-- <a href="" class="service-body-servicename">{{scope.row.metadata.name}}</a> -->
                                    <a class="service-body-servicename">{{scope.row.metadata.name}}</a>
                                </template>
                            </el-table-column>
                            <!-- 标签 -->
                            <el-table-column align=center label="标签">
                                <!-- 插槽，scope.row获取当前行的数据 -->
                                <template v-slot="scope">
                                    <!-- for循环，每个label只显示固定长度，鼠标悬停后气泡弹出框显示完整长度 -->
                                    <div v-for="(val, key) in scope.row.metadata.labels" :key="key">
                                        <!-- 气泡弹出框 -->
                                        <!-- placement 弹出位置 -->
                                        <!-- trigger 触发条件 -->
                                        <!-- content 弹出框内容 -->
                                        <el-popover 
                                        placement="right"
                                        :width="200"
                                        trigger="hover"
                                        :content="key + ':' + val">
                                            <template #reference>
                                                <el-tag style="margin-bottom:5px;" type="warning">{{ellipsis(key + ':' + val)}}</el-tag>
                                            </template>
                                        </el-popover>
                                    </div>
                                </template>
                            </el-table-column>
                            <!-- 类型 -->
                            <el-table-column align=center label="类型">
                                <!-- 可用数量/总数量,三元运算，若值大于0则显示值，否则显示0 -->
                                <template v-slot="scope">
                                    <span style="font-weight: bold;">{{ scope.row.spec.type }} </span>
                                </template>
                            </el-table-column>
                            <!-- cluster IP -->
                            <el-table-column align=center label="CLUSTER-IP">
                                <template v-slot="scope">
                                    <span>{{ scope.row.spec.clusterIP }} </span>
                                </template>
                            </el-table-column>
                            <!-- external IP -->
                            <el-table-column align=center label="EXTERNAL-IP">
                                <template v-slot="scope">
                                    <span>{{ scope.row.spec.externalIPs ? scope.row.spec.externalIPs[0] : '<none>' }} </span>
                                </template>
                            </el-table-column>
                            <!-- 端口 -->
                            <el-table-column align=center label="端口">
                                <template v-slot="scope">
                                    <span v-if="!scope.row.spec.ports[0].nodePort">{{ scope.row.spec.ports[0].port }}/{{ scope.row.spec.ports[0].protocol }}</span>
                                    <span v-if="scope.row.spec.ports[0].nodePort">{{ scope.row.spec.ports[0].port }}:{{ scope.row.spec.ports[0].nodePort }}/{{ scope.row.spec.ports[0].protocol }}</span>
                                </template>
                            </el-table-column>
                            <!-- 创建时间 -->
                            <el-table-column align=center min-width="100" label="创建时间">
                                <!-- timeTrans函数用于将格林威治时间转成北京时间 -->
                                <template v-slot="scope">
                                    <el-tag type="info">{{ timeTrans(scope.row.metadata.creationTimestamp) }} </el-tag>
                                </template>
                            </el-table-column>
            
                            <!-- 操作列，放按钮 -->
                            <el-table-column align=center label="操作" width="400">
                                <template v-slot="scope">
                                    <el-button size="small" style="border-radius:2px;" icon="Edit" type="primary" plain @click="getServiceDetail(scope)">YAML</el-button>
                                    <el-button size="small" disabled v-if="scope.row.metadata.name == 'kubernetes' " style="border-radius:2px;" icon="Delete" type="danger" @click="handleConfirm(scope, '删除', delService)">删除</el-button>
                                    <el-button size="small" v-else style="border-radius:2px;" icon="Delete" type="danger" @click="handleConfirm(scope, '删除', delService)">删除</el-button>
                                </template>
                            </el-table-column>
                        </el-table>
                        <!-- 分页配置 -->
                        <!-- background 背景色灰 -->
                        <!-- size-change 单页大小改变后触发 -->
                        <!-- current-change 页数改变后触发 -->
                        <!-- current-page 当前页 -->
                        <!-- page-size 单页大小 -->
                        <!-- layout 分页器支持的功能 -->
                        <!-- total 数据总条数 -->
                        <el-pagination 
                        class="service-body-pagination" 
                        background 
                        @size-change="handleSizeChange"
                        @current-change="handleCurrentChange"
                        :current-page="currentPage"
                        :page-sizes="pagesizeList"
                        :page-size="pagesize"
                        layout="total, sizes, prev, pager, next, jumper"
                        :total="serviceTotal">
                        </el-pagination>
                    </el-card>
                </div>
            </el-col>
        </el-row>

        

        <!-- =================================新建============================= -->
        <!-- 抽屉：创建Service的表单 -->
        <!-- v-model 值是bool，用于显示与隐藏 -->
        <!-- direction 显示的位置 -->
        <!-- before-close 关闭时触发，点击关闭或者点击空白都会触发 -->
        <el-Drawer v-model="createServiceDrawer"
        :direction="direction"
        :before-close="handleClose"
        size="40%">
            <!-- 插槽，抽屉标题 -->
            <template #header>
                <h4>创建Service</h4>
            </template>
            <!-- 插槽，body, 填写表单属性 -->
            <template #default>
                <!-- flex布局,居中 -->
                <el-row type="flex" justify="center">
                    <el-col :span="20">
                        <!-- ref绑定控件后，js中才能用this.$ref获取该控件 -->
                        <!-- rules 定义form表单校验规则 -->
                        <el-form ref="createService" :rules="createServiceRules" :model="createService" label-width="100px">
                            <!-- prop用于rules中的校验规则的key -->
                            <el-form-item class="service-create-form" label="名称" prop="name">
                                <el-input v-model="createService.name"></el-input>
                            </el-form-item>
                            <!-- 名称空间 -->
                            <el-form-item class="service-create-form" label="命名空间" prop="namespace">
                                <el-select v-model="createService.namespace" filterable placeholder="请选择">
                                    <el-option v-for="(item, index) in namespaceList" :key="index"
                                        :label="item.metadata.name" :value="item.metadata.name">
                                    </el-option>
                                </el-select>
                            </el-form-item>
                            <!-- 类型 Type -->
                            <el-form-item class="service-create-form" label="类型" prop="type">
                                <el-select v-model="createService.type" filterable placeholder="请选择">
                                    <el-option v-for="(item, index) in typeList" :key="index" :label="item" :value="item"></el-option>
                                </el-select>
                            </el-form-item>
                            <!-- 标签 -->
                            <el-form-item class="SERVICE-create-form" label="Service标签">
                                <el-input v-model="createService.label_str" placeholder="示例: project=ms,app=gateway"></el-input>
                            </el-form-item>
                            <!-- 标签选择器 -->
                            <el-form-item class="service-create-form" label="标签选择器" prop="selector_str">
                                <el-input v-model="createService.selector_str" placeholder="示例: project=ms,app=gateway"></el-input>
                            </el-form-item>
                            <!-- 端口名称 -->
                            <el-form-item class="service-create-form" label="端口名" prop="port_name">
                                <el-input v-model="createService.port_name" placeholder="示例: http"></el-input>
                            </el-form-item>
                            <!-- Service 端口 -->
                            <el-form-item class="service-create-form" label="Service端口" prop="port">
                                <el-input v-model="createService.port" placeholder="示例: 80"></el-input>
                            </el-form-item>
                            <!-- 目标端口（容器端口） -->
                            <el-form-item class="deploy-create-form" label="容器端口" prop="target_port">
                                <el-input v-model="createService.target_port" placeholder="示例: 80"></el-input>
                            </el-form-item>
                            <!-- 协议 -->
                            <el-form-item class="deploy-create-form" label="协议" prop="protocol">
                                <el-radio v-model="createService.protocol" label="TCP">TCP</el-radio>
                                <el-radio v-model="createService.protocol" label="UDP">UDP</el-radio>
                            </el-form-item>
                            <!-- LoadBalancerIP 如果选择为LoadBalancerIP类型，则弹出 -->
                            <el-form-item v-if="createService.type == 'LoadBalancer'" class="service-create-form" label="LoadBalancerIP" prop="load_balancer_ip">
                                <el-input v-model="createService.load_balancer_ip" placeholder="示例: 192.168.101.109"></el-input>
                            </el-form-item>
                            <!-- NodePort 如果选择为nodeport类型，则弹出 -->
                            <el-form-item v-if="createService.type == 'NodePort'" class="service-create-form" label="NodePort" prop="node_port">
                                <el-input v-model="createService.node_port" placeholder="示例: 30001，未填写则随机分配"></el-input>
                            </el-form-item>
                            <!-- ExternalIPs -->
                            <el-form-item class="SERVICE-create-form" label="ExternalIPs" prop="externalIPs">
                                <el-input v-model="createService.externalIPstr" placeholder="示例: 192.168.101.111,192.168.101.112"></el-input>
                            </el-form-item>

                        </el-form>
                    </el-col>
                </el-row>
            </template> 

            <!-- footer，处理提交和取消 -->
            <template #footer>
                <el-button @click="createServiceDrawer = false">取消</el-button>
                <el-button type="primary" @click="submitForm('createService')">立即创建</el-button>
            </template>
        </el-Drawer>


        <!-- =================================YAML============================= -->
        <!-- 展示YAML信息的弹框 -->
        <el-dialog
        title="YAML信息"
        v-model="yamlDialog"
        width="45%"
        top="2%">
            <!-- codemirror编辑器 -->
            <!-- border 带边框 -->
            <!-- options 编辑器配置 -->
            <!-- change 编辑器中的内容变化时触发 -->
            <codemirror 
            :value="contentYaml"
            border
            :options="cmOptions"
            height="500"
            style="font-size:14px;"
            @change="onChange"></codemirror>
            <template #footer>
                <span class="dialog-footer">
                    <el-button @click="yamlDialog = false">取 消</el-button>
                    <el-button type="primary" @click="updateService()">更 新</el-button>
                </span>
            </template>
        </el-dialog>



    </div>
</template>

<script>
import common from "../common/Config"
import httpClient from "@/utils/request"
import yaml2obj from "js-yaml"
import json2yaml from "json2yaml"

export default {
    data() {
        return {
            // 当前选中的cluster
            selectClusterValue: localStorage.getItem('k8s_cluster'),
            // 搜索
            searchInput: '',
            // 命名空间
            namespaceValue: 'default',
            namespaceList: [],
            namespaceListUrl: common.k8sNamespaceList,
            // 创建
            fullscreenLoading: false,
            createServiceDrawer: false,
            direction: 'rtl',
            // 创建service的属性
            typeList: ['ClusterIP', 'NodePort', 'LoadBalancer'],
            createService: {
                name: '',
                namespace: 'default',
                type: 'ClusterIP',
                selector_str: '',
                selector: {},
                port_name: '',
                port: '',
                target_port: '',
                protocol: 'TCP',
                node_port: '',
                load_balancer_ip: '',
                externalIPstr: '',
                externalIPs: [],
                label_str: '',
                label: {},
                cluster: '',
            },
            // 发送请求的参数
            createServiceData: {
                url: common.k8sServiceCreate,
                params: {},
            },
            // 创建service的表单校验规则
            createServiceRules: {
                name: [{
                    required: true,
                    message: '请填写名称',
                    trigger: 'change'
                }],
                namespace: [{
                    required: true,
                    message: '请选择命名空间',
                    trigger: 'change'
                }],
                type: [{
                    required: true,
                    message: '请选择类型',
                    trigger: 'change'
                }],
                selector_str: [{
                    required: true,
                    message: '请填写选择器',
                    trigger: 'change'
                }],
                port_name: [{
                    required: true,
                    message: '请填写端口名',
                    trigger: 'change'
                }],
                port: [{
                    required: true,
                    message: '请填写Service端口',
                    trigger: 'change'
                }],
                protocol: [{
                    required: true,
                    message: '请选择协议',
                    trigger: 'change'
                }],
                label_str: [{
                    required: true,
                    message: '请填写标签',
                    trigger: 'change'
                }],
                target_port: [{
                    required: true,
                    message: '请填写容器端口',
                    trigger: 'change'
                }],
            },

            // 表格列表
            serviceList: [],
            serviceTotal: 0, // 用于分页
            appLoading: false, // loading效果
            getServicesData: {
                url: common.k8sServiceList,
                params: {
                    filter_name: '',
                    namespace: '',
                    page: 1,
                    limit: 10,
                    cluster: '',
                }
            },
            // 分页
            currentPage: 1,
            pagesize: 10,
            pagesizeList: [10, 20, 30],

            // 编辑器配置
            cmOptions: common.cmOptions,
            contentYaml: '',
            // 详情
            yamlDialog: false,
            serviceDetail: {},
            getServiceDetailData: {
                url: common.k8sServiceDetail,
                params: {
                    service_name: '',
                    namespace: '',
                    cluster: '',
                }
            },
            // 更新
            updateServiceData: {
                url: common.k8sServiceUpdate,
                params: {
                    namespace: '',
                    content: '',
                    cluster: '',
                }
            },

            // 删除service
            deleteServiceData: {
                url: common.k8sServiceDel,
                params: {
                    namespace: '',
                    service_name: '',
                    cluster: '',
                }
            },

        }
    },
    methods: {
        // json转yaml
        transYaml(content){
            return json2yaml.stringify(content)
        },
        // yaml转对象
        transObj(content){
            return yaml2obj.load(content)
        },
        // 编辑器内容发生变换时触发的方法，用于将更新的内容复制到变量中
        onChange(val){
            this.contentYaml = val
        },
        // 分页, 页面大小发生变化时触发，赋值并重新获取列表
        handleSizeChange(size) {
            this.pagesize = size
            this.getServices()
        },
        // 页数发生变化时触发，复制并重新获取列表
        handleCurrentChange(currentPage){
            this.currentPage = currentPage
            this.getServices()
        }, 
        // 字符串截取、拼接并返回
        ellipsis(value){
            return value.length > 15 ? value.substring(0,15)+"...":value
        },
        // 字符串空格处理
        strTrimmed(value){
            return value.replace(/\s/g, "");
        },
        //格林威治时间转为北京时间
        timeTrans(timestamp) {
            let date = new Date(new Date(timestamp).getTime() + 8 * 3600 * 1000)
            date = date.toJSON();
            date = date.substring(0, 19).replace('T', ' ')
            return date 
        },

        // 获取Namespace列表
        getNamespaceList() {
            httpClient.get(this.namespaceListUrl, {params: {"cluster": this.selectClusterValue}}).then(res => {
                this.namespaceList = res.data.items
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
        },
        // 处理抽屉的关闭，doble check，增加体验
        handleClose(done){
            this.$confirm('确认关闭？').then(()=>{
                done();
            }).catch(()=>{})
        },

        // 重置表单方法 
        resetForm(formName) {
            this.$refs[formName].resetFields()
        },

        // 创建Service，提交表单，校验参数合法性
        submitForm(formName){
            this.$refs[formName].validate((valid)=>{
                if (valid) {
                    this.createServiceFunc()
                } else {
                    false
                }
            })
        },
        // 创建service，加Func的原因是因为createService用于属性了
        createServiceFunc(){
            // 正则匹配，验证标签选择弃和label的合法性
            let reg = new RegExp("(^[A-Za-z]+=[A-Za-z0-9]+).*")
            if (this.createService.label_str.length > 3 && !reg.test(this.createService.label_str)) {
                this.$message.warning({
                    message: "标签填写异常，请确认后重新填写"
                })
                return
            }
            if (!reg.test(this.createService.selector_str)){
                this.$message.warning({
                    message: "标签选择器填写异常，请确认后重新填写"
                })
                return
            }

            //加载loading动画
            this.fullscreenLoading = true

            // 定义label、selector
            // 'app=xxx,version=yyy'
            // ['app=xxx','version=yyy']
            // [['app','xxx'],['version','yyy']]
            // map['app']='xxx',map['version']=yyy
            let label = new Map()
            if (this.createService.label_str != ''){
                let a = (this.strTrimmed(this.createService.label_str)).split(",")
                a.forEach(item => {
                    let b = item.split("=")
                    label[b[0]] = b[1]
                })
            }
            let selector = new Map()
            let c = (this.strTrimmed(this.createService.selector_str)).split(',')
            c.forEach(item=>{
                let d = item.split('=')
                selector[d[0]] = d[1]
            })
            
            // 赋值
            this.createServiceData.params = this.createService
            this.createServiceData.params.selector = selector
            this.createServiceData.params.label = label
            this.createServiceData.params.port = parseInt(this.createService.port)
            this.createServiceData.params.target_port = parseInt(this.createService.target_port)
            this.createServiceData.params.node_port = parseInt(this.createService.node_port)
            this.createServiceData.params.cluster = this.selectClusterValue
            // 处理externalIPs
            if (this.createService.externalIPstr !== ''){
                this.createServiceData.params.externalIPs =  this.strTrimmed(this.createService.externalIPstr).split(',')
            }

            // 发起请求
            httpClient.post(this.createServiceData.url, this.createServiceData.params).then(res=>{
                this.$message.success({
                    message: res.msg
                })
                // 创建后重新获取列表
                this.getServices()
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
            
            // 重置表单（只能重置必填项的内容）
            this.resetForm('createService')
            // 清空非必填项的内容
            this.createService.label_str = ''
            this.createService.node_port = ''
            this.createService.load_balancer_ip = ''
            this.createService.externalIPs = []

            // 关闭加载动画
            this.fullscreenLoading = false
            // 关闭抽屉
            this.createServiceDrawer = false
        },
        // 获取Service列表
        getServices() {
            // 表格加载动画开启
            this.appLoading = true
            //getServicesData是用于发起service列表请求的专用的对象，里面有url和params参数,以下是赋值
            this.getServicesData.params.filter_name = this.searchInput
            this.getServicesData.params.namespace = this.namespaceValue
            this.getServicesData.params.page = this.currentPage
            this.getServicesData.params.limit = this.pagesize
            this.getServicesData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getServicesData.url, {params: this.getServicesData.params}).then(res => {
                this.serviceList = res.data.items
                this.serviceTotal = res.data.total
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
            this.appLoading = false
        },
        // 获取Service详情，e参数标识传入的scope插槽，.row是该行的数据
        getServiceDetail(e){
            //getServicesData是用于发起service列表请求的专用的对象，里面有url和params参数,以下是赋值
            this.getServiceDetailData.params.namespace = this.namespaceValue
            this.getServiceDetailData.params.service_name = e.row.metadata.name
            this.getServiceDetailData.params.cluster = this.selectClusterValue
            // 发起请求获取数据
            httpClient.get(this.getServiceDetailData.url, {params: this.getServiceDetailData.params}).then(res => {
                //响应成功，获得service详情
                this.serviceDetail = res.data
                //将对象转成yaml格式的字符串
                this.contentYaml = this.transYaml(this.serviceDetail)
                //打开弹出框
                this.yamlDialog = true
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
        },
        // 更新Service
        updateService(){
            //将yaml格式的service对象转为json
            let content = JSON.stringify(this.transObj(this.contentYaml))
            this.updateServiceData.params.content = content
            this.updateServiceData.params.namespace = this.namespaceValue
            this.updateServiceData.params.cluster = this.selectClusterValue
            httpClient.put(this.updateServiceData.url, this.updateServiceData.params).then(res => {
                this.$message.success({
                    message: res.msg
                })
                //更新后重新获取列表
                this.getServices()
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
            //关闭弹出框
            this.yamlDialog = false
        },

        // 删除service
        delService(e){
            this.deleteServiceData.params.namespace = this.namespaceValue
            this.deleteServiceData.params.service_name = e.row.metadata.name 
            this.deleteServiceData.params.cluster = this.selectClusterValue
            httpClient.delete(this.deleteServiceData.url, {data: this.deleteServiceData.params}).then(res => {
                this.$message.success({
                    message: res.msg
                })
                // 删除后重新获取列表
                this.getServices()
            }).catch(res => {
                this.$message.error({
                    message: res.msg
                })
            })
        },
        // 弹出确认框，用于危险操作的double check
        // obj 是行数据，opeateName是操作名，fn是操作的方法
        handleConfirm(obj, operateName, fn){
            this.confirmContent = '确认继续' + operateName + '操作吗？'
            // $confirm用于弹出确认框
            this.$confirm(this.confirmContent, '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
            }).then(()=>{
                //restartService(e)或者delService(e)
                fn(obj)
            }).catch(()=>{
                this.$message.info({
                    message: '已取消操作'
                })
            })
        },
    },

    watch: {
        //监听namespace的值,若发生变化，则执行handler方法中的内容
        namespaceValue: {
            handler(){
                //将namespace的值存入本地，用于path切换时依旧能获取得到
                localStorage.setItem('namespace', this.namespaceValue)
                //重置当前页为1
                this.currentPage = 1
                //获取service列表
                this.getServices()
            }
        },
        // 监听clustervalue的值，这里表示如果localstorage中的值发生了改变，则重新获取掉接口刷新当前页面
        selectClusterValue: {
            handler(){
                this.getNamespaceList()
                this.getServices()
            }
        }
    },
    mounted(){     
        //需要将vue实例赋值给_this,这样在window.addEventListener中通过_this可以为vue实例上data中的变量赋值
        let _this=this;
        window.addEventListener("setItemEvent", function(e){
             //e.key : 是值发生变化的key
            //e.newValue : 是可以对应的新值
            if(e.key==="k8s_cluster"){
                _this.selectClusterValue = e.newValue
            }
        })
    },
    beforeMount() {
        //加载页面时先获取localStorage中的namespace值，若获取不到则默认default
        if (localStorage.getItem('namespace') !== undefined && localStorage.getItem('namespace') != null) {
            this.namespaceValue = localStorage.getItem('namespace')
        }
        this.getNamespaceList()
        this.getServices()
    }
}
</script>


<style scoped>
/* 卡片属性 */
.service-head-card,
.service-body-card {
    border-radius: 1px;
    margin-bottom: 5px;
}
/* 搜索框 */
.service-head-search{
    width: 160px;
    margin-right: 10px;
}
/* 数据表格service名颜色 */
.service-body-servicename {
    color: #4795EE;
}
    /* service名鼠标悬停 */
.service-body-servicename:hover {
    color: rgb(84, 138, 238);
    cursor: pointer;
    font-weight: bold;
}
</style>